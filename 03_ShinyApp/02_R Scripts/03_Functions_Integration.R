...
integrated.shiny.app <- function(countyID,barID,outcomeID,demogrID,yaxisID){
  if (outcomeID == 1){
    ...
    if (yaxisID %in% (1:4) && demogrID == 1){
      plot.shiny.app.injury(countyID = countyID,barID = barID,yaxisID = yaxisID)
    }
    # #Throws error if Injury is selected for Physical Activity Outcome
    # else if (yaxisID %in% (1:4) && demogrID == 2){
    #   plot.shiny.app.injury(countyID = PhysicalActivity,barID = barID,yaxisID = yaxisID)
    # }
    # #Throws error if Injury is selected for Physical Activity Outcome
    # else if (yaxisID %in% (1:4) && demogrID == 3){
    #   plot.shiny.app.injury(countyID = PhysicalActivity,barID = barID,yaxisID = yaxisID)
    # }
    # #Throws error if Injury is selected for Physical Activity Outcome
    # else if (yaxisID == 5){plot.shiny.app.injury(countyID = PhysicalActivity,barID = barID,yaxisID = yaxisID)}
